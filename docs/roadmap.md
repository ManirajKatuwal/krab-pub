...
- Frontend SSR/islands runtime remains on **Axum**, with route registration generated by [services/service_frontend/build.rs](../services/service_frontend/build.rs).
- Shared DB lifecycle primitives remain centralized in [crates/framework/krab_core/src/db/](../crates/framework/krab_core/src/db/).

This boundary is treated as **non-negotiable for phases 1–4**, except via ADR + owner signoff.

> **Amended.** This item previously named `crates/framework/krab_server/src/lib.rs`
> as the SSR runtime. That crate never handled a request — `build.rs` generated
> Axum registration from the day it was written — and it was removed under
> [ADR 0005](adr/0005-krab-server-disposition.md). Its static-path traversal
> defence was ported to `krab_core::static_assets` first. The DB path also moved:
> `db.rs` became the `db/` module directory when SQLite was promoted into the
> framework.
